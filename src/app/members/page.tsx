          ...
          <div className="space-y-2">
            <AnimatePresence mode="popLayout">
              {filtered.map((member, i) => (
                ...
                >
                  <div className="w-11 h-11 rounded-full bg-linear-to-br from-[#c8f65d]/20 to-[#c8f65d]/5 flex items-center justify-center text-[#c8f65d] font-bold text-sm shrink-0">
                    {member.name[0]}
                  </div>
...
export default function MembersPage() {
    return (
        <Suspense fallback={<div className="flex items-center justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-[#c8f65d]" /></div>}>
            <MembersContent />
        </Suspense>
    );
}
