  ...
  if (!member) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => router.push("/members")} className="text-[#c8f65d] text-sm hover:underline">Back to Members</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <button onClick={() => router.push("/members")} className="flex items-center gap-2 text-white/50 hover:text-white transition-colors text-sm">
          <ArrowLeft size={16} />
          Back to Members
        </button>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#161616] border border-white/5 rounded-2xl p-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-linear-to-br from-[#c8f65d]/30 to-[#c8f65d]/10 flex items-center justify-center text-3xl font-bold text-[#c8f65d]">
              {member.name[0]}
            </div>
          ...
          <div className="space-y-3">
            {memberPayments.map((payment, i) => (
              <motion.div key={payment.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }} className="flex items-center gap-4 py-3 border-b border-white/5 last:border-0">
                <div className="w-9 h-9 rounded-xl bg-[#c8f65d]/10 flex items-center justify-center shrink-0">
...
function Modal({ children, title }: { children: React.ReactNode; onClose: () => void; title: string }) {
  return (
